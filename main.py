...
async def status():
    await bot.change_presence(activity=discord.Activity(type=discord.ActivityType.watching, name=f"Over {len(bot.users)} people in {len(bot.guilds)} servers"), status = discord.Status.idle)

...
class Dono(commands.Cog):
    ...
    async def dono(self, ctx, member: discord.Member):
        ...
        embed.add_field(name='Giveaway Donations:', value=f'`{result[0]}` donated for giveaways in **{ctx.guild.name}**', inline=False)
    ...
    async def dono_add(self, ctx, member: discord.Member, amount: int):
        ...
        cursor.execute("INSERT INTO gaw_dono_logs (guild_id, user_id, amount) VALUES (?, ?, ?) ON CONFLICT DO UPDATE SET amount = amount + ?;", [guild, user, amount, amount])

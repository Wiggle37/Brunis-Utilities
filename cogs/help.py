...
class Help(commands.Cog):
    ...
    async def help(self, ctx, cog=None):
        ...
        if cog == 'dono' or cog == 'donation':
            ...
            help_embed.add_field(name='Dono_remove', value='Removes from someones donation amount```b!dr <@member / id> <amount(int)>```')
            help_embed.add_field(name='Dono_set(coming soon)', value='Sets the amount to a certain member```b!ds <@member /id> <amount(int)>```')
            await ctx.send(embed=help_embed)
